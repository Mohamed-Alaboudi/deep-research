...
Central claims are verified by `dr-verifier` subagents in a **batched escalation ladder** (web/knowledge/mixed modes): Round 1 is 3 verifiers at thorough (claims split evenly) / ≤10-per-batch at lite/standard; only important or contradicted claims get a 2nd/3rd batched re-check; unresolved contradictions are thrown out of the findings. Claim caps per tier: lite 5, standard 10, thorough 25. Verdicts are balanced (confirmed / uncertain / contradicted, no default-refute). Knowledge mode's mandatory fact-check is satisfied by this stage. Full detail: `references/verification.md`.
